...
const PlantDetailsModal: React.FC<PlantDetailsModalProps> = ({ plant, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
      <div className="bg-[var(--surface)] w-full max-w-2xl max-h-[90vh] overflow-y-auto rounded-3xl shadow-2xl border border-[var(--border)] relative">
        <button 
          onClick={onClose}
          className="absolute top-6 right-6 p-2 rounded-full bg-[var(--bg)] border border-[var(--border)] hover:bg-[var(--surface)] transition-colors z-10"
        >
        ...
        <div className="relative h-64 w-full">
          {plant.image ? (
            ...
            <div className="w-full h-full bg-[var(--bg)] flex items-center justify-center text-6xl">🌱</div>
          )}
          <div className="absolute inset-0 bg-gradient-to-t from-[var(--surface)] to-transparent" />
          <div className="absolute bottom-6 left-8">
            <h2 className="text-3xl font-bold text-[var(--text)]">{plant.common_name}</h2>
            {plant.scientific_name && (
              <p className="text-lg italic opacity-60">{Array.isArray(plant.scientific_name) ? plant.scientific_name[0] : plant.scientific_name}</p>
            )}
          </div>
        </div>
